...
PROG = cachesim
TARBALL = $(if $(USER),$(USER),jroot30)-proj1.tar.gz
...
submit: clean
	tar --exclude=project1_description.pdf -czhvf $(TARBALL) run.sh run_experiment.sh Makefile $(wildcard *.xlsx *.pdf *.cpp *.c *.hpp *.h) 
	@echo
